      ...
      <div class="flex" v-for="pos in posList">
        ...
        <div class="flex gap-3 flex-wrap items-end">
          <span v-for="tran in pos.trans">
            <span v-if="!settingStore.dictation || props.showFull" :class="tran.frequency != undefined?['rare', 'uncommon', 'common'][tran.frequency]:''">{{ tran.cn }}</span>
            <SentenceHightLightWord v-else :text="tran.cn" :word="word.word" :dictation="true" :high-light="false" />
          </span>
        </div>
      </div>
    </div>
